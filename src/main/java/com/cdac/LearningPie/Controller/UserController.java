...
//		
		try {
			FileCopyUtils.copy(profilePicDetails.getProfilePic().getInputStream(), new FileOutputStream("F:\\FINAL PROJECT\\Front End git\\learningpie\\public\\Uploaded\\ProfilePic\\"+fileName));
		} catch (IOException e) {
...
//		return status;
	...
	public int updatePassword(@RequestBody VerifyOtpDto verifyOtpDto) {
		return userService.updatePassword(verifyOtpDto.getOtp(),verifyOtpDto.getPassword(),verifyOtpDto.getUserName());
	}
    ...
    @PostMapping("/findbyusernameandemail")
	public User findByUserNameAndEmail(@RequestBody User user) {
		return userService.findByUserNameAndEmail(user.getUserName(), user.getEmail());
