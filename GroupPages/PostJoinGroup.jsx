...
export default function PostJoinGroup() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem('user') == null) {
      ...
      sessionStorage.getItem('joingroupId') == null &&
      ...
      // setUserId(uId);
      let gId = parseInt(doDecryption(sessionStorage.getItem('joingroupId')))
      console.log(gId)
      getData(gId)
      // setgroupId(gId);
    }
  ...
  const submit = (Question) => {
    axios
      .post(`http://localhost:8080/postQuestionInGroup/${groupId}`, Question)
      .then(
        (response) => {
  ...
  const getData = async (gId) => {
    const response = await axios.get(
      `http://localhost:8080/getGroupInfo/${gId}`
    )
    let validQuestions = []
  ...
  return (
    <>
      <div
        style={{
          backgroundImage: 'url(https://wallpapercave.com/wp/wp9098575.jpg)',
